...
async function runStep(config: InventorConfig): Promise<InventorStepResult> {
  ...
  // ── Auto-Linear Issue: High-scoring artifacts → production proposals ──
  const PRODUCTION_THRESHOLD = 0.7
  if (result.success && result.score >= PRODUCTION_THRESHOLD) {
    try {
      const isUrgent = result.score >= 0.85
      const issueTitle = `[Inventor] ${config.experimentName}: ${config.taskDescription.slice(0, 60)}...`
      const issueBody = `## 🧬 Inventor Proposal — Production Candidate

**Score:** ${result.score.toFixed(3)} / 1.0 (${result.success ? '✅ PASS' : '❌ FAIL'})
...
### Metrics
- Precision: ${result.metrics.precision?.toFixed(2) ?? 'N/A'}
- Reasoning: ${result.metrics.reasoning?.toFixed(2) ?? 'N/A'}
- Information: ${result.metrics.information?.toFixed(2) ?? 'N/A'}
- Safety: ${result.metrics.safety?.toFixed(2) ?? 'N/A'}
- Methodology: ${result.metrics.methodology?.toFixed(2) ?? 'N/A'}

### Parent Lineage
${parentId ? `- Parent: ${parentId} (score: ${nodes.get(parentId)?.score?.toFixed(3) ?? 'N/A'})` : '- Seed node (no parent)'}

### Neo4j Query
\`\`\`cypher
MATCH (t:InventorTrial {id: '${nodeId}'}) RETURN t
\`\`\`
`
      await callMcpTool({
        ...
        args: {
          ...
          team: 'widgetdc-orchestrator',
          priority: isUrgent ? 1 : 2, // Urgent or High
          labels: [
            'inventor-proposal',
            result.score >= 0.85 ? 'score-a' : result.score >= 0.7 ? 'score-b' : 'score-c',
            `experiment:${config.experimentName}`,
          ],
        },
      ...
      logger.info(
        { nodeId, score: result.score, isUrgent, experiment: config.experimentName },
        'Inventor: Auto-created Linear issue for production candidate'
      )
      broadcastMessage({
        from: 'Inventor',
        to: 'All',
        source: 'orchestrator',
        type: 'Message',
        message: `🧬 Inventor proposal: ${issueTitle} (score: ${result.score.toFixed(3)})`,
        timestamp: new Date().toISOString(),
      } as Record<string, unknown>)
    } catch (linearErr) {
      logger.warn({ nodeId, error: String(linearErr) }, 'Inventor: Failed to create Linear issue (non-critical)')
    }
  }
