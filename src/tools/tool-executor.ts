...
import { verifyChainOutput } from '../chain/verification-gate.js'
import { runInvestigation } from '../chain/investigate-chain.js'
...
async function executeToolByName(name: string, args: Record<string, unknown>): Promise<string> {
  switch (name) {
    case 'search_knowledge': {
      ...
      try {
        const { hookAutoEnrichment } = await import('../swarm/compound-hooks.js')
    ...
    // Both names handled: 'graph.write_cypher' from REST gateway and 'write_cypher'
    // from LOCAL_TOOLS (mcp-caller strips namespace prefix before calling executeToolUnified).
    ...
    case 'write_cypher': {
      ...
      const { neo4jDirectWrite } = await import('../neo4j-direct.js')
      return await neo4jDirectWrite(query, params)
    }
    ...
    case 'ingest_document': {
      ...
      try {
        const { ingestDocument } = await import('../engagement/document-intelligence.js')
    ...
    case 'build_communities': {
      try {
        const { buildCommunitySummaries } = await import('../graph/hierarchical-intelligence.js')
    ...
    case 'adaptive_rag_dashboard': {
      try {
        const { getAdaptiveRAGDashboard } = await import('../memory/adaptive-rag.js')
    ...
    case 'graph_hygiene_run': {
      try {
        const { runGraphHygiene } = await import('../graph/graph-hygiene-cron.js')
    ...
    case 'precedent_search': {
      ...
      try {
        const { findSimilarClients } = await import('../engagement/similarity-engine.js')
    ...
    case 'governance_matrix': {
      const { getEnforcementMatrix, getEnforcementScore, getGaps } = await import('../llm/manifesto-governance.js')
    ...
    case 'adaptive_rag_retrain': {
      try {
        const { retrainRoutingWeights } = await import('../memory/adaptive-rag.js')
    ...
    case 'adaptive_rag_reward': {
      ...
      try {
        const { sendQLearningReward } = await import('../memory/adaptive-rag.js')
    ...
    case 'moa_query': {
      ...
      try {
        const { routeMoA } = await import('../llm/moa-router.js')
    ...
    case 'critique_refine': {
      ...
      if (!query || query.length < 5) return 'Error: query is required (min 5 chars)'
      try {
        const { critiqueRefine } = await import('../llm/critique-refine.js')
    ...
    case 'forge_tool': {
      ...
      try {
        const { forgeTool, verifyForgedTool } = await import('../llm/skill-forge.js')
    ...
    case 'forge_analyze_gaps': {
      try {
        const { analyzeToolGaps } = await import('../llm/skill-forge.js')
    ...
    case 'forge_list': {
      try {
        const { getForgedTools } = await import('../llm/skill-forge.js')
    ...
    case 'engagement_create': {
      try {
        // v4.0.11: apply same validation as /api/engagements route — prevents empty records
        // in Redis when tool is called via /api/tools/* or /mcp (omega-test-003 P2 finding).
        ...
        const { createEngagement } = await import('../engagement/engagement-engine.js')
        const result = await createEngagement({
          ...
          budget_dkk: typeof args.budget_dkk === 'number' && args.budget_dkk >= 0 ? args.budget_dkk : undefined,
          team_size: typeof args.team_size === 'number' && args.team_size > 0 && args.team_size < 500 ? args.team_size : undefined,
    ...
    case 'engagement_match': {
      try {
        const { matchPrecedents } = await import('../engagement/engagement-engine.js')
    ...
    case 'engagement_plan': {
      try {
        const { generatePlan, PlanGateRejection } = await import('../engagement/engagement-engine.js')
    ...
    case 'engagement_outcome': {
      try {
        const { recordOutcome } = await import('../engagement/engagement-engine.js')
    ...
    case 'engagement_list': {
      try {
        const { listEngagements } = await import('../engagement/engagement-engine.js')
    ...
    case 'memory_store': {
      try {
        const { storeMemory } = await import('../memory/working-memory.js')
    ...
    case 'memory_retrieve': {
      try {
        const { retrieveMemory, listMemories } = await import('../memory/working-memory.js')
        const agentId = String(args.agent_id ?? '')
    ...
    case 'research_harvest': {
      try {
        const { executeChain } = await import('../chain/chain-engine.js')
    ...
    case 'hyperagent_auto_run': {
      try {
        const { runAutonomousCycle } = await import('../hyperagent/hyperagent-autonomous.js')
        ...
        const { persistCrossRepoMemory } = await import('../hyperagent/hyperagent-autonomous.js')
    ...
    case 'hyperagent_auto_status': {
      try {
        const { getAutonomousStatus } = await import('../hyperagent/hyperagent-autonomous.js')
    ...
    case 'hyperagent_auto_memory': {
      try {
        const { persistCrossRepoMemory, readCrossRepoMemory, listCrossRepoMemory } = await import('../hyperagent/hyperagent-autonomous.js')
    ...
    case 'pheromone_sense': {
      const { sense } = await import('../swarm/pheromone-layer.js')
    ...
    case 'pheromone_deposit': {
      const { deposit } = await import('../swarm/pheromone-layer.js')
    ...
    case 'pheromone_heatmap': {
      const { getHeatmap } = await import('../swarm/pheromone-layer.js')
    ...
    case 'peer_eval_status': {
      const { getPeerEvalState } = await import('../swarm/peer-eval.js')
    ...
    case 'peer_eval_fleet': {
      ...
      if (taskType) {
        const { getFleetLearning, getWhatWorks } = await import('../swarm/peer-eval.js')
    ...
    case 'peer_eval_analyze': {
      const { runFleetAnalysis } = await import('../swarm/peer-eval.js')
...
ORDER BY n.score DESC LIMIT ${maxItems}`,
    ...
    case 'chat_read': {
      ...
      const { config } = await import('../config.js')
      const headers = { 'Authorization': `Bearer ${config.orchestratorApiKey}`, 'Accept': 'application/json' }
      let messages: any[] = []

      if (args?.thread_id) {
        // Use dedicated thread endpoint — reads all 2000 Redis entries, not just recent N
        const res = await fetch(`http://localhost:${config.port}/chat/threads/${encodeURIComponent(String(args.thread_id))}`, {
          headers,
          signal: AbortSignal.timeout(10000),
        })
        const data = await res.json().catch(() => ({}))
        ...
        // General history — newest first
        const fetchLimit = args?.from_agent ? 500 : limit
